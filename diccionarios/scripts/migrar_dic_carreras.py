...
def migrate_dic_carreras_fast():
    ...
    try:
        ...
        print("Extrayendo datos de 'dicCarreras' desde MySQL...")
        mysql_cursor.execute("SELECT Id, IdFacultad, Nombre FROM dicCarreras;")
        ...
        # Añadir el valor por defecto para estado_carrera
        ...
        columns = ('id', 'id_facultad', 'nombre', 'estado_carrera')
        pg_cursor.copy_expert(f"COPY public.dic_carreras ({', '.join(columns)}) FROM STDIN WITH CSV DELIMITER AS E'\t'", buffer)

        # 4. Confirmar la transacción
        pg_conn.commit()
        
        print(f"¡Éxito! Se han insertado {len(transformed_data)} registros en 'dic_carreras' usando COPY.")
        ...
        if pg_conn:
            pg_conn.close()
        print("--- Migración de Carreras finalizada ---")
