...
def migrate_dic_obtencion_studios_from_csv():
    """
    Migra datos desde un CSV sin ID a la tabla dic_obtencion_studios,
    dejando que el ID sea autogenerado.
    """
    print("--- Iniciando migración de Obtención de Estudios desde CSV ---")
    
    pg_conn = None
    
    csv_file_path = os.path.join(os.path.dirname(__file__), '..', 'dic_obtencion_studios_rows.csv')
    ...
    try:
        ...
        with open(csv_file_path, 'r', encoding='utf-8') as f:
            ...
            for row in reader:
                # row[0] = nombre, row[1] = descripcion
        ...
        sql_copy = f"COPY public.dic_obtencion_studios ({', '.join(columns)}) FROM STDIN WITH CSV DELIMITER AS E'\t'"
        ...
        print(f"¡Éxito! Se han cargado {len(transformed_data)} registros en 'dic_obtencion_studios'.")
        ...
        if pg_conn:
            pg_conn.close()
        print("--- Migración de Obtención de Estudios finalizada ---")
