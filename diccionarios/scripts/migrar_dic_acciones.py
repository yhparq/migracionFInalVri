...
def migrate_dic_acciones_from_csv():
    """
    Migra datos desde un archivo CSV (dic_acciones_rows.csv) a la tabla
    dic_acciones en PostgreSQL usando el método de carga masiva COPY.
    """
    print("--- Iniciando migración de Acciones desde CSV ---")
    
    pg_conn = None
    
    csv_file_path = os.path.join(os.path.dirname(__file__), '..', 'dic_acciones_rows.csv')
    ...
    try:
        ...
        print("¡Éxito! Se han cargado los datos en 'dic_acciones' usando COPY.")
        ...
        if pg_conn:
            pg_conn.close()
        print("--- Migración de Acciones finalizada ---")
