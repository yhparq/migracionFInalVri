...
def migrate_tbl_asignacion_fast():
    """
    Migra el historial de cambios de jurado desde tesJuCambios (MySQL) a 
    tbl_asignacion (PostgreSQL) utilizando el método COPY para una carga masiva de alto rendimiento.
    """
    print("--- Iniciando migración de tbl_asignacion (Método COPY) ---")
    pg_conn = None
    mysql_conn = None
    try:
        ...
        if data_for_copy:
            ...
            pg_cur.copy_expert(
                sql=f"COPY public.tbl_asignacion ({', '.join(columns)}) FROM STDIN WITH (FORMAT CSV, DELIMITER E'\t', NULL '\\N')",
                file=buffer
            )
        ...
        print("--- Migración de tbl_asignacion completada con éxito ---")
